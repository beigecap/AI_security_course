...
1. **Базовая модель демонстрирует высокую уязвимость к атакам.**  
   Attack Success Rate (89,6%) показывает, что в большинстве случаев модель выполняет вредоносные инструкции вместо отказа.

2. **Добавление safety prompt снижает ASR, но не решает проблему полностью.**  
   Несмотря на более строгие правила (изоляция данных, отказ от roleplay, защита от injection), ASR снизился лишь до 82,8%, что указывает на ограниченную эффективность prompt-level защиты.
